...
//var_dump($session_user_image);
//var_dump($session_user_image."---".$session_username);
        ...
        <div class="row">
            ...
            <div class="col-md-9">
                <h1>
                    ...
                    <small class="text-black-50">Add New Post</small>
                </h1>
                <hr/>
                <ol class="breadcrumb">
                    <li><a href="index.php"><i class="fa fa-tachometer-alt"></i> Dashboard /</a></li>
                    <li class="active pl-2"><i class="fas fa-comments"></i> Add Post</li>
                </ol>
                ...
                if (isset($_POST["submit"])) {
                    $post_date = time();
                    $post_title = mysqli_real_escape_string($connectionDB, $_POST["title"]);
                    $post_data = mysqli_real_escape_string($connectionDB, $_POST["post-data"]);
                    $post_category = $_POST["categories"];
                    $post_tags = $_POST["tags"];
                    $post_status = $_POST["status"];
                    $post_image = $_FILES["image"]["name"];

                    if (empty($post_title) || empty($post_data) || empty($post_category) || empty($post_tags) || empty($post_status) || empty($post_image)) {
                        $error_message = "All (*) fields are required!!!";
                    } else {
                        $imageArr = explode('.', $post_image);
                        $rand = rand(10000, 99999);
                        $newPostImageName = $imageArr[0] . '_' . $rand . '_' . $post_date . '.' . $imageArr[1];
                        ...
                        $insert_query = "INSERT INTO posts (date, title, author, author_image, image, categories, tags, post_data, views, status) 
                                            VALUES ('$post_date', '$post_title', '$session_username', '$session_user_image', '$newPostImageName', '$post_category', 
                                            '$post_tags', '$post_data', '0', '$post_status')";
                        ...
                        if (mysqli_query($connectionDB, $insert_query)) {
                            ...
                            $error_message = "Something wrong, post has not been added!!!";
                        }
                    }
                }
                ?>
                        ...
                        <form action="" method="post" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                <textarea name="post-data" id="textarea" rows="10" cols="30"
                                          class="form-control"><?php if (isset($post_title)) echo $post_title; ?></textarea>
                            </div>
                            ...
                            <input type="submit" class="btn btn-primary float-right" name="submit" value="Add Post">
                        </form>
                    </div>
                </div>
            </div>
